...
ALPHA_ZERO_CONFIG = {
    ...
    # 手札予測ヘッド（Belief）設定
    # ---------------------------
    # True でモデルに相手手札(各カード53枚)所持確率出力ヘッドを追加
    ...
    "mcts_early_stop_visit_ratio": 0.70,     # ルート最大訪問子 / 総訪問 >= ratio で候補 (0.70→0.80で厳格化)
    "mcts_early_stop_gap_ratio": 0.12,       # (top - second)/総訪問 >= gap なら確定 (0.12→0.15で厳格化)
    ...
    "eval_gate_start_after_updates": 2000,
    # 直近の評価開始からこの更新数に達するまで次の評価を起動しない（0/未設定で無効）
    "eval_gate_every_updates": 2000,
    ...
    # --- 計測/メトリクス ---
    # ネットワークの1回forward時間(t_forward)とゲーム時間/手数を計測するか
    ...
    "concurrent_min_new_samples_before_train": 12000,  # メモリ削減: 学習頻度を下げて蓄積抑制
    ...
    "replay_memory_cooldown_sec": 600,
    ...
    # 非同期リプレイ挿入設定
    # ---------------------------
    # True で append を専用スレッドへ委譲し MCTS/ゲームループの待ち時間を低減
    "replay_async_enabled": True,
    # 非同期キュー最大サイズ (オーバーしたら古い/新規を破棄)。大き過ぎるとメモリ肥大
