...
IF NOT DEFINED EPISODES_PER_GEN SET EPISODES_PER_GEN=200
IF NOT DEFINED WORKERS SET WORKERS=16
IF NOT DEFINED TRAIN_UPDATES SET TRAIN_UPDATES=200
IF NOT DEFINED MAX_FILES_PER_TRAIN SET MAX_FILES_PER_TRAIN=40
REM data/ の保存上限（FIFO）。100を既定に設定
IF NOT DEFINED DATA_MAX_FILES SET DATA_MAX_FILES=150
...
REM Loop count
SET LOOP_COUNT=%1
IF "%LOOP_COUNT%"=="" SET LOOP_COUNT=2500
...
ECHO [PHASE self-play] episodes=%EPISODES_PER_GEN% workers=%WORKERS%
"%PYTHON_EXE%" -m non_parallel_trainer.non_parallel_self_play --episodes !EPISODES_PER_GEN! --workers !WORKERS! --data-dir "!DATA_DIR!" --log-dir "!LOG_DIR!" --checkpoint-dir "!CKPT_DIR!"
...
ECHO [PHASE train] updates=%TRAIN_UPDATES% max-files=%MAX_FILES_PER_TRAIN%
"%PYTHON_EXE%" -m non_parallel_trainer.non_parallel_trainer --data-dir "!DATA_DIR!" --log-dir "!LOG_DIR!" --checkpoint-dir "!CKPT_DIR!" --updates !TRAIN_UPDATES! --max-files !MAX_FILES_PER_TRAIN! --version-interval !VERSION_INTERVAL!
